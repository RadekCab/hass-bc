...
class SimUser:
    """Class used for creating model of simulated user
    used in training of learning agent
    """

    def __init__(
        self, name, leave: list, arrive: list, targets: dict = {}, is_sim=True
    ):
        self._setting = 1
        # sorted!
    ...
    def get_user_request_per_timeframe(
        self, time_index, temperature_index, tolerance=0.5
    ):
        ...
        if self._setting == 1:
            if time_index in self.target_temp_time:
                if (
                    index_to_temperature(temperature_index)
                    > self.target_temp_time[time_index] + 0.5
                ):
                    # decrease
                ...
                latest_request_time = self._get_latest_request_time(
                    time_index, self.is_sim
                )
                if latest_request_time is not None:
                    latest_target_temperature = self.target_temp_time[
                        latest_request_time
                    ]
                    if (
                        index_to_temperature(temperature_index)
                        > latest_target_temperature + tolerance
                    ):
                        return TemperatureAction.DECREASE
                    if (
                        index_to_temperature(temperature_index)
    ...
    def _get_latest_request_time(self, time_index, sim=True):
        """
        can be used in real time (use sim=False), but will work with time plan

        Args:
            time_index (_type_): real time converted to 0-95 interval
            sim (bool, optional): if sim is True, first target temperature
                                                    is taken from the nearest future

        """
        tg_time = list(self.target_temp_time.keys())
        time_index_present = np.searchsorted(
            self._present_time, time_index, side="left"
        )
