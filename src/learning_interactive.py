...
from events import (
    nearest_pointfive,
    exp_increase_array_at_indeces,
    generate_cold_requests,
)
...
__doc__ = """
Interactive file, testing implemented fucntionalities
Reuqires jupyter to be installed, do so installing 
the requirements.txt requirements
"""
...
def to_insert_reflex_agent_inputs():
    ...
    for i in range(Inhabitant.get_request_count()):
        time_index, request = Inhabitant.get_requests_by_target_temp(temperature_day, i)

        temperature_day = exp_increase_array_at_indeces(
            temperature_day, {time_index: request}
        )
...
Inhabitant.process_temperature_targets_from_intervals(
    DEFAULT_TEMPERATURE, heat_indices, noheat_indices
)
...
TemperatureEnvironment = SimTemperature(
    DEFAULT_TEMPERATURE, after_actions, exp_init=True
)
...
optimal_temp_actions = create_plan(
    DEFAULT_TEMPERATURE, result_q_table, Inhabitant, TemperatureEnvironment, mode=""
)
...
print(
    f"User wants {str(timedelta(minutes=int(0) * 15))}-{str(timedelta(minutes=int(Inhabitant.leave[0]) * 15))}"
