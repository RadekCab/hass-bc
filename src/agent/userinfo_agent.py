...
class UserAgent(DeviceAgent):
    def __init__(self, users : list, devices=[], jid="userinfo11@sure.im", password="userinfo66", topic="") -> None:
    ...
    class RecvTimeplanNoHeatBehav(PeriodicBehaviour):
        ...
        async def run(self):
           ...
           # print("User Agent Got Thread Priority...")
        ...
        async def activate_mqqt_topic_listening(self, topic):
            ...
            async with sem:
                #print("NoHeat Got Thread Priority...")
        ...
        async def run(self):
            ...
            if msg:
                if self.template.body == "timeplan":
                    ...
                    reply = Message(to=str(self.template.sender),
    ...
    class ParametersCheckBehav(PeriodicBehaviour):
        async def run(self):
            #print("Current Saved TimePlan Requests (Heat,Noheat):",
            #      self.get(PROPERTY_TIMEPLAN_HEAT), self.get(PROPERTY_TIMEPLAN_NOHEAT))
            pass
