...
class TemperatureAgent(DeviceAgent):
    ...
    class RecvTemperatureRequestBehav(PeriodicBehaviour):
        ...
        async def run(self):
            #print("Temperature subscribed to Learning")
            temperature = self.get("temperature")
            if temperature is not None and temperature != "nothing":
                #print("Sending temperature to learning agent that is online.")
        ...
        async def activate_mqqt_topic_listening(self, topic):
            async with AsyncioPahoClient(str(uuid.uuid4())) as client:
                ...
                if client._userdata == "new":
                    ...
                    # TODO immedieate reaction for this interval
                    # TODO if first in this interval, learning should re-learn with this request
    ...
    class RoomTemperatureReceiveBehav(PeriodicBehaviour):
        ...
        async def run(self):
            ...
            async with sem:
                #print("Temperature Agent Got Thread Priority...")
                await self.activate_mqqt_topic_listening(self.get("mqtt_temperature_topic"))

        async def activate_mqqt_topic_listening(self, topic):
            #async with AsyncioPahoClient(str(self.get("uuid"))) as client:
            async with AsyncioPahoClient(str(uuid.uuid4())) as client:
                #print(topic)
                client._userdata = None
                client.username_pw_set(username="hass",password="nimda")
                ...
                client.user_data_set("nothing")
            #self.kill
            #TODO send to learning agent
            
    class RecvLearningRequestsBehav(CyclicBehaviour):
        async def run(self):
            #print("Receiving from learning...")
            msg = await self.receive(timeout=11)  # wait for a message for 10 seconds
            #print("Temperature Agent: Listening for message", self.template.body)
            if msg:
                temperature = self.get("temperature")
                #print("Temperature agent: Solving message body, template:", self.template.body)
                # TODO kdyby chyba tak tady podminka nova
                if temperature is not None and temperature != "nothing" and self.template.body == "temperature":
                    ...
                    await self.send(msg)
                    #print("Temperature Agent: temperature sent", msg)
                elif self.template.body == "temperature":
                    print("Temperature Agent: cannot send temperature, I dont know it:", temperature)
    ...
    class SolveActionForNewTimeIntervalBehav(OneShotBehaviour):
        ...
        async def run(self):
            ...
            #q_table = np.fromfile("qtable.bin", like=np.zeros((2, 96, 21, 3, 3)))
            if not os.path.exists("q_table.npy"):
                await asyncio.sleep(30)
            q_table = np.load("q_table.npy")
            print("Temperature Agent: Q-Table Obtained From File")
            #print(q_table)
            action = get_next_temperature_action(presence, derivative, time_interval,
                                                temperature_to_index(float(temperature)), 1.,1.,q_table)
            print("Temperature Agent: Next Action Should Be", TemperatureAction(action).name, ".")
    ...
    class SimTimeIntervalCheckBehav(PeriodicBehaviour):
        async def run(self):
            ...
            if self.get("time") == "sim":
                ...
                #print("Current Time Interval " + str(index) + "vs last " + str(self.get("current_time_interval")))
                new_interval = False
                print("Time:", str(timedelta(minutes=self.get("current_time_interval")*15)))
                ...
                if new_interval or self.get("start_interval"):
                    ...
                    #next_action_solve_behav = self.get("process_next_action_behav")
                    b =  self.agent.SolveActionForNewTimeIntervalBehav()
                    #self.agent.add_behaviour(self.agent.next_action_behav)
    ...
    class ParameterSelfCheckBehav(PeriodicBehaviour):
        async def run(self):
            ...
            inc_counter = self.get("temperature_inc_req_cnt")
            dec_counter = self.get("temperature_dec_req_cnt")
            #print(f"TA: Received {inc_counter} Increase Requests and {dec_counter} Decrease Requests")
            if self.get("temperature_inc_req") and int(inc_counter) <= 1:
                ...
                #print("Temperature Agent: Too many Increase requests in one time interval, working with first one.")
            ...
            if self.get("temperature_dec_req") and int(dec_counter) <= 1:
                ...
                #print("Temperature Agent: Too many Decrease requests in one time interval, working with first one.")
    ...
    async def setup(self):
        ...
        #self.set("process_next_action_behav", process_new_time_interval_behav)
        ...
        #self.add_behaviour(self.RecvTemperatureRequestBehav(period=10))
        ...
        #TODO check for q_table
        #TODO get current time a work with it
        #TODO if temperature changed, check if do action
